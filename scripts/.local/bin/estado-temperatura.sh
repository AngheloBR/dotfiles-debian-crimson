...
#
# Busca el sensor por NOMBRE y no por numero de hwmon, porque
# /sys/class/hwmon/hwmonN cambia de numero entre arranques:
#   k10temp  = CPU AMD (esta laptop)
#   coretemp = CPU Intel
#   acpitz   = sensor generico ACPI (ultimo recurso)
...
for d in /sys/class/hwmon/*; do
    case "$(cat "$d/name" 2>/dev/null)" in k10temp|coretemp|acpitz) ;; *) continue;; esac
